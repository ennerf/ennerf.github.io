...
While both interfaces have strengths and weaknesses, practically all use cases can be solved using either one. For this project we chose to use the Java interface because it needed to support all major operating systems, and Java greatly simplifies cross-platform support as well as the deployment of binaries. The diagram below shows an overview of the resulting system.
