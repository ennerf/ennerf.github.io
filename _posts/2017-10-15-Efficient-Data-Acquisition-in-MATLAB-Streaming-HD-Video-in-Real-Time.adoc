...
While both interfaces have strengths and weaknesses, practically all use cases can be solved using either one. For this project we chose to use the Java interface because we needed to support all major operating systems, and using Java greatly simplified cross-platform development as well as the deployment of binaries. The diagram below shows an overview of the resulting system.
