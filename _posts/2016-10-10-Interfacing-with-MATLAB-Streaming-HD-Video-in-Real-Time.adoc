...
The results show that the default conversion as well as jpeg compression are essentially non-starters for color images. The default approach works much better for grayscale due the conversion from a 2D array (`byte[height][width]`) being significantly cheaper, but it still is not a great option.
