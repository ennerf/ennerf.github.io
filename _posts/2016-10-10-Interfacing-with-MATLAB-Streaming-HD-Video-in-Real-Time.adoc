...
For us, the biggest lesson learned was that shared memory can be used to efficiently transfer large amounts of data into MATLAB. We also found that shared memory can serve as a convenient way to exchange data between multiple MATLAB instances. For example, one instance can track a target at a lower rate, while another instance can use its output for robot control at a higher rate.
