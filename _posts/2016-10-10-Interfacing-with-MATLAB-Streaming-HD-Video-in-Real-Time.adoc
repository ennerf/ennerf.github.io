...
* changing the type back to `uint8` gets somewhat messy because the `uint8(matrix)` cast would set all negative values to zero, and the alternative `typecast(matrix, 'uint8')` only works on vectors

Thus, converting the data to a valid image matrix still required several operations.
