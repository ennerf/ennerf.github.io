...
NOTE: TODO: should this blog be about real-time control or on latency with a focus on robotics?
...
Let's first look at the stock performance of consumer operating systems: Windows, Mac and Linux. I've done a clean install for each OS, disabled sleep mode, and performed any steps necessary to start jHiccup. There were no other modifications. After starting, I've unplugged all cables other than the power cable and let the computer sit idle for >24 hours.
