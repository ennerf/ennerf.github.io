...
The first important realization when looking at latency is that data is not normally distributed. I have seen many data sets where the worst observed case was more than 1000 standard deviations away from the mean. Looking at only the mean and standard deviation tends to provide an extremely optimistic view that can be misleading.
...
However, before dismissing consumer OS for real-time control systems entirely, it is worth looking at their actual performance. Even though there are no theoretical guarantees, the practical performance may be good enough for many applications.
