...
[NOTE]
====
Dave: Putting a figure up here, even if its a dummy figure showing what you mean in terms of outliers or non-gaussian, and since you mention that visual tools like histograms and percentile plots are important. Maybe a histogram and percentile distribution of the windows latency data?  That might also help the flow into the next section.
====
