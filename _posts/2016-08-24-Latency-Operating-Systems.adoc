...
However, commodity operating systems can significantly ease development. Before prematurely dismissing them for real-time control systems, it is worth looking at their actual performance. Even though there are no theoretical guarantees, the practical differences may not be noticeable for many applications.
