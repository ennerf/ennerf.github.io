...
I'm not very familiar with tuning systems at this level, so I'm probably missing a few things. There are various tuning guides, e.g., Red Hat's link:http://linux.web.cern.ch/linux/mrg/2.3/Red_Hat_Enterprise_MRG-2-Realtime_Tuning_Guide-en-US.pdf[MRG Realtime Tuning Guide], that can provide more information. The process priority was set to 98, which is for high priority real-time threads (see link:https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_MRG/2/html/Realtime_Tuning_Guide/chap-Realtime-Specific_Tuning.html#Setting_scheduler_priorities[setting scheduler priorities]).
