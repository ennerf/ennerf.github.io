...
:page-image: { path: /images/jfx-static/sampler-blueprints.png, alt: "The AtlantaFX sampler as a JavaFX 27 Native Image on a Raspberry Pi 5" }
:page-description: How Gluon Substrate, Liberica NIK, and StaticFX get JavaFX running as a GraalVM Native Image, including WebView and media, benchmarked on a Raspberry Pi 5.
...
In fact, starting the https://github.com/mkpaz/atlantafx[AtlantaFX] sampler on a high-end Ryzen 9 9950X desktop with jlink + CDS takes more than 2.5 times as long (1.3 s) as the same application running AOT-compiled on a tiny Raspberry Pi 5 (0.5 s, see <<benchmarks,benchmarks>>). The whole run is captured in a short https://youtu.be/GA_iAnxznO8[video walkthrough] of all pages, including FXML, WebView, MediaPlayer, and AWT integration.
...
By default, missing items can fail silently or with rather unhelpful runtime errors, e.g., a missing FXML file:
...
GraalVM for JDK 23 added the opt-in `--exact-reachability-metadata` build flag that turns missing registrations into detailed instructions:

[source]
----
Caused by: com.oracle.svm.core.jdk.resources.MissingResourceRegistrationError: Cannot access resource at path 'sampler/hello.png'. To allow this operation, add the following to the 'resources' section of 'reachability-metadata.json' and rebuild the native image:

    {
      "glob": "sampler/hello.png"
    }
----

Note that this also fails when looking up optional files that don't exist, such as the binary `.bss` versions that JavaFX checks whenever loading a stylesheet. Fortunately, globs are accepted (only in `-Areachability.outputFormat=1.2.0`), so this can be worked around with a single `@Reachable(resources = "/sampler/**/*.bss")`.
...
* Overloaded native methods never link: native-image resolves builtin JNI methods by their short name, and an overloaded native only exists under its signature-mangled name on the C side, as the JNI spec requires (relevant to three classes). We added `@CFunction` substitutions to manually route them to the mangled symbols. BellSoft works around this with source patches that rename the overloaded natives (e.g., `CreateFontFace` -> `CreateFontFaceIndexed`), and Substrate is unaffected due to lazy resolution on Windows and macOS (falls back to the mangled names at runtime), and none of the classes being relevant on Linux.
