...
First, we can get an idea of the best-case baseline by having two devices communicate with each other through a single switch and without any external traffic. It'd be possible to omitt the switch entirely by creating a point-to-point connection, but that would be a relatively rare use case for us. 

I set the frequency to 100Hz assuming that the round trip time for one measurement would never exceed 10ms. 
