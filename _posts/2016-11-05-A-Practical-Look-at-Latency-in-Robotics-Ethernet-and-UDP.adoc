...
There are several competing networking standards that are built on Ethernet and can guarantee enough determinism to be used in industrial automation (https://en.wikipedia.org/wiki/Industrial_Ethernet[Industrial Ethernet]). They achieve this by enforcing tight control over the network layout and by limiting the components that can be connected. However, even cheap consumer grade network equipment can produce very good results if the network is controlled in a similar manner.
 ...
 TODO: Add that this insight is not new and that people have been doing this for decades. Find good posts and papers to cite.

//  TODO: add that this insight is not new and that people have been doing this in 2001. Link various old posts and papers? Are there good ones?

// http://www.embedded.com/design/connectivity/4023291/Real-Time-Ethernet[Joe Kerkes] tried this in 2001 using 10 Mbit Ethernet.

// https://www.researchgate.net/publication/4232548_Real-time_performance_measurements_using_UDP_on_Windows_and_Linux[Prytz et al.] did a latency study in 2005. Other potentially useful resources include a list of

// http://literature.rockwellautomation.com/idc/groups/literature/documents/wp/enet-wp002_-en-p.pdf[differences between Standard Ethernet and proprietary adaptations].    

// http://www.embedded.com/electronics-blogs/cole-bin/4406659/1/Deterministic-networking--from-niches-to-the-mainstream-[Deterministic Networking: from niches to the mainstream? (2013)] 
...
Our startup http://hebirobotics.com/[HEBI Robotics] builds a variety of building blocks that enable quick development of custom robotic systems. We mainly focus on actuators, but we also developed the I/O Board shown in the picture above. Each board has 48 pins that serve a variety of functions (analog and digitial I/O, PWM, Encoder input, etc.) that can be accessed remotely via network. We normally use them in conjunction with our actuators to interface with external devices, such as a gripper or pneumatic valve, or to get various sensor input into MATLAB.
