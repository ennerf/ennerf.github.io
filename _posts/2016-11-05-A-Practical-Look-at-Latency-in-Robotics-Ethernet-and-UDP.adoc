...
Most of the UDP benchmarks that I was able to find online setup two Desktop computers, where the sender sends a message, the receiver echoes it back, and the sender measures the time it took for a round trip. This works well, but there is a lot of 'magic' that happens in the underlying operating system and it's device drivers.

In order to mitigate this potential source of jitter, I setup a benchmark that measures the round trip time between two dedicated embedded systems. Aside from being on an embedded system, the approach is the same, i.e.,
...
All of the benchmarks in this post measure the round-trip time for a UDP datagram with a payload of 200 bytes. After including all overhead, the actual frame size on the wire is 266 bytes. For comparison, the feedback packets of HEBI Robotic's X-Series actuators currently require a payload of 185-215 bytes. Requesting the state of all pins of an I/O board currently requires about 300 bytes.

While this size is representative of our standard use case, the round trip times in practice are slightly faster because the outgoing packets (commands) are usually significantly smaller than response packets (feedback).
...
Both KSZ8863 and the RB750Gr2 add a constant switching latency of 3.4 us and 3.7 us in addition to the wire time of 4 us and 40 us respectively to the RTT. The added jitter seems to be negligible at well below 1us.
...
In order to test this hypothesis we put together a test setup consisting of 40 HEBI Robotics IO boards that are connected to a  http://www.downloads.netgear.com/files/GDC/GS748Tv1/GS748T_ds_03Feb05.pdf[GS748T] 48-port Ethernet switch.

image::udp/multiple-boards.jpg[]

The idea is to send a single broadcast message that gets received by all devices at the same time. This would then trigger 40x 266 byte (>10KB) return packets that often arrive at the switch within less than 250 nanoseconds. This would be representative of a very high degree of freedom system such as a full body humanoid robot.

In order to keep the sender device from overloading, I changed the sender to a Desktop computer running https://ennerf.github.io/2016/09/20/A-Practical-Look-at-Latency-in-Robotics-The-Importance-of-Metrics-and-Operating-Systems.html[Scientific Linux 6.6 with real-time kernel]. Let's again take a look at the jitter of the underlying operating system. The figure below shows the difference between an idealized signal that ticks every 10ms and the actual measurements of the loop start times. It shows that this setup suffers from more than an order of magnitude more jitter than the embedded OS.

[NOTE]
The corresponding jHiccup control chart looks identical to the OS blog post.

image::udp/os-jitter-linux-rt.png[]

Gathering data at 100Hz close to 90 minutes resulted in more than 20 million returned packets. Again, there were no packet drops or re-ordering of packets from a single source. I also ran some tests at 1KHz and the result looks the same.

image::udp/rtt-linux-40x-zoomed.png[]

Rather than blowing up, what actually happens is that all packets get stored in the internal buffer of the switch (1.632MB), queued, and forwarded to the target port as fast as possible. Since the original sender is connected via 1 Gigabit, the packets arrive every ~2us. All of them have the same start timestamp, so the graph below shows this as vertical columns.

image::udp/rtt-linux-40x.png[]

What surprised me is that the latency is so much higher than the previous tests. Looking at the previous tests, I would have expected most columns to start at around ~180us and end at ~280us. While this is sometimes the case, the majority of columns start above 300 us. After some initial research I suspect that this is related to the Linux https://en.wikipedia.org/wiki/New_API[NAPI] using polling mode rather than working with interrupts.
...
Even consumer-grade Ethernet networks are actually very deterministic. Re-ordering is not an issue on local networks and unless ports get saturated, there is no packet loss.

Almost all of the commonly observed jitter is caused by the operating system and it's device drivers. The chart below compares the Linux system communicating (using unicast) with a single device in the same setup as the baseline test.

image::udp/rtt-linux-1x.png[]

[NOTE]
TODO: fix the plot and extend text.

I can't recommend using UDP for controlling extremely dangerous industrial robots, but just about everything else should be fine.
