...
* On macOS, the process hangs before the first window appears. Cocoa requires the process's first thread to be running a https://developer.apple.com/documentation/corefoundation/cfrunloop[CFRunLoop] while glass starts up. The `java` launcher handles this, but a native image runs `main` on the first thread. We can substitute a handoff to cover `Application::launch`, but `Platform::startup` currently needs an external launcher.
