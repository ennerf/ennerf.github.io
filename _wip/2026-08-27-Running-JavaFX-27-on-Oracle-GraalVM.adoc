...
* `System.loadLibrary` cannot initialize any of the static JavaFX libraries. https://openjdk.org/jeps/178[JEP 178] requires statically linked libraries to report at least `JNI_VERSION_1_8`, but glass still reports older versions and fails with `UnsatisfiedLinkError: Unsupported JNI version 0x10006, required by glass`. On Linux and macOS, the `JNI_OnLoad_<lib>` entry points are additionally hidden by the image's exported-symbol list, so the feature calls the initializers directly instead.
...
The media and web modules do not build static archives because their native dependencies (GStreamer and WebKit) only exist as shared libraries. However, on a JVM these modules already load their libraries dynamically from the platform jars, and nothing prevents a native image from doing the same. The feature simply copies the matching `.dll/.so/.dylib` from the classpath to the output, and adds appropriate linker arguments. Applications that don't use media or web remain standalone executables.
...
After the initial class loading, the JIT closes most of the gap on the desktop, while the Pi stays noticeably faster under AOT. It is also noticeable that FXML loads significantly faster on AOT on both platforms, even after multiple calls.
